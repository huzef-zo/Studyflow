...
## 2026-07-24 - [Local Storage Exhaustion and DoS via Unbounded Storage Mutations]
...
## 2026-09-25 - Numeric Ingestion Bounds Bypassing in Backup Import
**Vulnerability:** Application state corruption and calculation freezes via unvalidated non-finite numbers (`Infinity`, `NaN`, negative numbers, ultra-large values) in `Storage.importData`.
**Learning:** `Storage.importData` relied on simplistic `Number(...)` casting for session `duration`, task `progress`, task `sortOrder`, subtask `estimatedCycles`/`completedCycles`, and `xpState` metrics (`totalXP`, `currentLevel`, `xpGained`). Malicious backup files containing `Infinity` or negative numbers bypassed downstream validation in direct mutation APIs (like `addSession`), persisting invalid numbers in local storage that broke analytics and UI rendering.
**Prevention:** Always validate all imported numeric properties with `Number.isFinite()` and explicitly clamp values to expected min/max domain bounds directly inside data ingestion routines before writing to storage.
