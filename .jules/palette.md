...
## 2026-08-23 - [Header Action Containers & Calendar Strip Layout]
**Learning:** Appending dynamically injected buttons (such as theme toggles) directly into flex header containers with existing action buttons can cause unintended line breaks or full-width wrapping on small screens. Wrapping header action controls in a dedicated `.page-header-actions` container allows buttons to stay neatly grouped and flex side-by-side. Additionally, ensuring `.calendar-strip` uses `display: flex; gap: ...; overflow-x: auto;` and `flex: 1` on items prevents multi-day indicators from stacking vertically as massive full-width cards.
**Action:** Group header action buttons inside a `.page-header-actions` wrapper and update theme toggle injection logic to append into `.page-header-actions` when present. Define `.calendar-strip` as a horizontal flex container to display calendar strip days in a clean horizontal row.
