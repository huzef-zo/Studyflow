...
## 2026-09-25 - [Raw Storage Access & Inline Date Formatting for Analytics]
**Learning:** High-level list getters like `Storage.getTasks()` resolve daily completion state for every repeating task in storage. In historical analytics functions (such as `getFilteredTasks()`, `getCompletedTasksInPeriod()`, and `getActivityData()`), this today-state resolution is completely redundant because historical completion state is evaluated separately across custom date ranges. Querying raw task storage (`Storage.loadData(KEYS.TASKS)`) directly and formatting `YYYY-MM-DD` strings inline in date loops bypasses redundant task transformations and `Storage.formatDate` overhead, reducing analytics rendering duration by ~40%.
**Action:** In historical analytics and time-range filters where today's completion state is superseded by range queries, access raw collection storage directly and format date range strings inline.
